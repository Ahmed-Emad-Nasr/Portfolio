...
export const useGitHubRepos = (): {
  ...
  useEffect(() => {
    ...
    const fetchRepos = async (): Promise<void> => {
      ...
      const tryUseCachedRepos = (message: string, allowStale = false): boolean => {
        const cached = readCachedRepos();
        if (!cached) return false;
        if (!cached.isFresh && !allowStale) return false;
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            if (tryUseCachedRepos("GitHub returned empty data. Showing cached repositories.") || tryUseCachedRepos("GitHub returned empty data. Showing cached repositories.", true)) {
              return;
            }
          ...
          if (isLastAttempt) {
            ...
            if (
              tryUseCachedRepos("Live GitHub data is unavailable. Showing cached repositories.") ||
              tryUseCachedRepos("Live GitHub data is unavailable. Showing cached repositories.", true)
            ) {
              return;
            }
  ...
  return { repos, isLoading, source, loadError, cacheUpdatedAt, refresh };
};